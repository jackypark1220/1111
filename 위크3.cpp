...
void drawcircle(const int x_circle, const int y_circle, const int r, const float& red, const float& green, const float& blue)
{
	int x_c = x_circle;
	int y_c = y_circle;
	...
	for (int a = 0; a <= 2 * r; a++)
	{
		for (int b = 0; b <= 2 * r; b++)
		{
			...
			if (f < -30&& f>-350)
			{
				drawPixel(x_circle - r + b, y_circle - r + a, 255, 0, 0);
			}
		}
	}
}
